    ...
    $.fn.tabullet = function (options) {
        ...
        columns.each(function (i, v) {
            metadata.push({
                ...
                hidden: $(v).attr('data-tabullet-hidden'),
                insert: $(v).attr('data-tabullet-insert'),
        ...
        $(data).each(function (i, v) {
            ...
            $(metadata).each(function (mi, mv) {
                if (mv.type === 'delete') {
                    var td = $("<td/>")
                        .html('<button class="' + options.deleteClass + '"><i class="fas fa-trash"></i></button>')
                    ...
                    var td = $("<td/>")
                        .html('<button class="' + options.editClass + '"><i class="fas fa-edit"></i></button>')
                        .attr('data-tabullet-type', mv.type)
                        .appendTo(tr);
                    td.find('button').click(function (event) {
                        ...
                        $(rowChildren).each(function (ri, rv) {
                            insert = $(rv).attr('data-tabullet-map');
        ...
        // INSERT
        var insertRow = $("table").data('tabullet-insert') == false
            ? "" : $("<tr/>").appendTo($(tbody)).attr('data-tabullet-id', "-1");
